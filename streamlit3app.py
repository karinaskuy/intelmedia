...
* **AI-Driven Insights (Gemini):** A dedicated function, `generate_insights_from_gemini`, makes API calls to the Gemini model. This function extracts 3 concise insights for each chart, giving you quick takeaways from your data.

**Important Note on API Key:**

The persistent **"401 Unauthorized" errors** you encountered previously indicate that your Gemini API Key is not being correctly provided to the application. In this Streamlit version, the API key is retrieved from your **environment variables**.
...
const generateInsights = useCallback(async (chartName, dataToAnalyze, setStateCallback) => {
  ...
  try {
    ...
    // Sekarang, URL-nya akan mengambil nilai dari environment variable
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`;
